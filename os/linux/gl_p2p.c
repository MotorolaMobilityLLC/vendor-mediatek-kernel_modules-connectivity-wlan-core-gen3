...
BOOLEAN p2PAllocInfo(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	do {
		if (prGlueInfo->prP2PInfo == NULL) {
			...
			prWifiVar->prP2pQueryStaStatistics = kalMemAlloc(sizeof(PARAM_GET_STA_STA_STATISTICS),
									 VIR_MEM_TYPE);
		} else {
		...
		if (prWifiVar->prP2pQueryStaStatistics)
			kalMemZero(prWifiVar->prP2pQueryStaStatistics, sizeof(PARAM_GET_STA_STA_STATISTICS));
...
/* prWifiVar->prP2pFsmInfo && */
	...
	if (prWifiVar->prP2pQueryStaStatistics) {
		kalMemFree(prWifiVar->prP2pQueryStaStatistics, VIR_MEM_TYPE, sizeof(PARAM_GET_STA_STA_STATISTICS));
...
/* kalMemFree(prGlueInfo->prAdapter->rWifiVar.prP2pFsmInfo, VIR_MEM_TYPE, sizeof(P2P_FSM_INFO_T)); */
		...
		kalMemFree(prGlueInfo->prAdapter->rWifiVar.prP2pQueryStaStatistics, VIR_MEM_TYPE,
...
/* prGlueInfo->prAdapter->rWifiVar.prP2pFsmInfo = NULL; */
		...
		prGlueInfo->prAdapter->rWifiVar.prP2pQueryStaStatistics = NULL;
...
BOOLEAN glRegisterP2P(P_GLUE_INFO_T prGlueInfo, const char *prDevName, BOOLEAN fgIsApMode)
{
...
#if CFG_ENABLE_WIFI_DIRECT_CFG_80211
	if (!gprP2pWdev) {
		DBGLOG(P2P, ERROR, "p2p wireless device is not exist\n");
		return FALSE;
	}
#endif
	...
	if (!prGlueInfo->prAdapter->fgEnable5GBand)
		gprP2pWdev->wiphy->bands[KAL_BAND_5GHZ] = NULL;
